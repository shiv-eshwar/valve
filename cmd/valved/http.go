...
func (b checkBody) toAPI() (api.Key, api.Limits, api.Cost) {
	return api.Key{Subject: b.Key.Subject, Model: b.Key.Model},
		api.Limits{RequestsPerMinute: b.Limits.RequestsPerMinute, TokensPerMinute: b.Limits.TokensPerMinute},
		api.Cost{Requests: b.Cost.Requests, Tokens: b.Cost.Tokens}
}
...
func (a *httpAPI) settle(w http.ResponseWriter, r *http.Request) {
	...
	d, err := a.lim.Settle(r.Context(), body.ReservationID, body.ActualTokens)
	if err != nil {
		http.Error(w, err.Error(), http.StatusBadRequest)
		return
	}
